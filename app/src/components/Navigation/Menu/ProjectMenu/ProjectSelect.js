...
export default function ProjectSelect() {
  ...
  const renderMenuItems = () => {
    return projectsLoading ? <ProjectSelectSkeleton/> : projects.map(project => {
      return <MenuItem value={project._id} key={project._id} onClick={() => handleClose(project._id)}
                       selected={currentProject && currentProject._id === project._id}>{project.name}</MenuItem>
    });
  }
    ...
    <>
      ...
      >
        ...
        <MenuItem onClick={() => {
          ...
          handleMenuClose();
        }}><Typography color="primary">Create new project</Typography></MenuItem>
      </Menu>
    </>
  )
}
